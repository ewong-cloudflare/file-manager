...
import { FolderOpen, FileText, Download, RefreshCw, Loader2, Inbox } from "lucide-react";
import { listSharedWithMe, getSharedItem } from "../lib/api";
...
export function SharedWithMe({ onToast }: SharedWithMeProps) {
  ...
  async function handleOpen(share: ShareRecord) {
    setDownloadingId(share.id);
    try {
      const res = await getSharedItem(share.link_token);
      if (res.downloadUrl) {
        window.open(res.downloadUrl, "_blank", "noopener,noreferrer");
      } else {
        onToast("Folder browsing coming soon", "error");
      }
    } catch (err) {
      onToast(err instanceof Error ? err.message : "Failed to open shared item", "error");
    } finally {
              ...
              <tr key={share.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                ...
                <td className="px-5 py-3 text-right">
                  ...
                  >
                    ...
                    {share.is_folder ? "Browse" : "Download"}
                  </button>
                </td>
              </tr>
            ))}
